...
function AdminPage() {
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto">
      <PageHeader title="Administration" subtitle="Users, roles, teams, categories and system configuration" />

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <KpiCard label="Users" value={users.length} icon={Users} tone="info" />
        <KpiCard label="Active Roles" value={4} icon={ShieldCheck} sub="Engineer · Manager · Executive · Admin" />
      ...
      <div className="border-b border-border mb-6 flex gap-1 overflow-x-auto">
        {TABS.map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 text-sm border-b-2 -mb-px whitespace-nowrap ${tab === t ? "border-primary text-foreground font-medium" : "border-transparent text-muted-foreground hover:text-foreground"}`}>{t}</button>
        ))}
      </div>
      ...
      {tab === "Users" && (
        <div className="rounded-lg border border-border bg-card overflow-hidden">
          <div className="px-4 py-3 border-b border-border flex items-center justify-between">
            ...
            <button className="inline-flex items-center gap-2 rounded-md bg-primary text-primary-foreground px-3 py-1.5 text-sm hover:bg-primary/90"><Plus className="h-4 w-4" /> Add User</button>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wider text-muted-foreground border-b border-border bg-muted/30">
                <th className="px-4 py-2.5">Name</th><th className="px-4 py-2.5">Username</th>
                <th className="px-4 py-2.5">Role</th><th className="px-4 py-2.5">Team</th>
      ...
      {tab === "Roles" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {[
            { role: "Engineer", count: 6, desc: "Operational front-line · executes work, submits handover" },
            { role: "Manager", count: 1, desc: "Command view · assigns work, audits handover and approvals" },
          ...
          {["DC", "NOC", "Shared"].map((t) => (
            <div key={t} className="rounded-lg border border-border bg-card p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {users.filter((u) => u.team === t).map((u) => (
                  <span key={u.id} className="text-xs rounded-full bg-muted px-2 py-0.5">{u.name.split(" ")[0]}</span>
                ))}
              </div>
            </div>
          ))}
      ...
      {(tab === "Categories" || tab === "Shift Settings" || tab === "System") && (
        <div className="rounded-lg border border-border bg-card p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Configuration UI for {tab.toLowerCase()} — mock placeholder.</p>
        </div>
      )}
    </div>
  );
