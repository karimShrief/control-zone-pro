...
function ProjectDetail() {
  ...
  const subs = projectTasks.filter((pt) => pt.projectId === project.id);

  return (
    <div className="p-6 max-w-[1600px] mx-auto">
      <Link to="/projects" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-3">
      ...
      <div className="border-b border-border mb-6 flex gap-1">
        {TABS.map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 text-sm border-b-2 -mb-px ${tab === t ? "border-primary text-foreground font-medium" : "border-transparent text-muted-foreground hover:text-foreground"}`}>{t}</button>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="rounded-lg border border-border bg-card p-4">
              ...
              <div className="grid grid-cols-3 gap-3 mt-4 text-sm">
                <div><div className="text-xs text-muted-foreground">Start</div><div className="font-medium">{project.startDate}</div></div>
                <div><div className="text-xs text-muted-foreground">Target</div><div className="font-medium">{project.targetDate}</div></div>
                <div><div className="text-xs text-muted-foreground">Risk</div><StatusBadge status={project.risk} /></div>
              </div>
            </div>
            {project.blockers.length > 0 && (
              <div className="rounded-lg border border-critical/30 bg-critical/5 p-4">
                ...
                <ul className="space-y-1 text-sm">
                  {project.blockers.map((b) => <li key={b}>• {b}</li>)}
                </ul>
              </div>
            )}
          </div>
          <div className="space-y-4">
            <div className="rounded-lg border border-border bg-card p-4 space-y-2 text-sm">
              ...
              <Field label="Priority"><StatusBadge status={project.priority} /></Field>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wider text-muted-foreground border-b border-border bg-muted/30">
                <th className="px-4 py-2.5">ID</th><th className="px-4 py-2.5">Title</th>
                <th className="px-4 py-2.5">Assignee</th><th className="px-4 py-2.5">Status</th>
                <th className="px-4 py-2.5">Priority</th><th className="px-4 py-2.5">Due</th>
            ...
            <tbody className="divide-y divide-border">
              {subs.map((pt) => (
                <tr key={pt.id} className="hover:bg-muted/40">
                  ...
                  <td className="px-4 py-3"><StatusBadge status={pt.priority} /></td>
                  <td className="px-4 py-3 text-xs text-muted-foreground">{pt.dueDate}</td>
                  <td className="px-4 py-3 w-40">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden"><div className="h-full bg-primary" style={{ width: `${pt.completion}%` }} /></div>
          ...
          {(["To Do", "In Progress", "Review", "Blocked", "Done"] as const).map((col) => (
            <div key={col} className="rounded-lg border border-border bg-card p-3">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs text-muted-foreground">{subs.filter((s) => s.status === col).length}</span>
              </div>
              <div className="space-y-2">
                {subs.filter((s) => s.status === col).map((s) => (
                  <div key={s.id} className="rounded-md border border-border bg-background p-2.5">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">{userById(s.assignee).split(" ")[0]}</span>
            ...
            {subs.map((s) => (
              <div key={s.id} className="flex items-center gap-4">
                ...
                <div className="flex-1 h-8 bg-muted rounded-md relative overflow-hidden">
                  <div className="absolute inset-y-0 left-0 bg-primary/30" style={{ width: `${s.completion}%` }} />
                  <div className="absolute inset-0 flex items-center px-3 text-xs font-medium">{s.title}</div>
                </div>
