  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto">
      <PageHeader
        ...
        subtitle="Morning and Night shifts · clock in/out and live roster"
      />
                ...
                <div className="space-y-1.5">
                  {todays?.engineers.map((id) => {
                    ...
                    return (
                      <div
                        key={id}
                        className="flex items-center justify-between text-sm rounded-md border border-border bg-background px-3 py-1.5"
                      >
                        <div className="flex items-center gap-2">
                          <span
                            className={`h-2 w-2 rounded-full ${isOn ? "bg-success animate-pulse" : isDone ? "bg-info" : "bg-muted-foreground/40"}`}
                          />
                          {userById(id)}
                        </div>
                        <span className="text-xs text-muted-foreground tabular-nums">
                          {r?.signInAt
                            ? `${fmtTime(r.signInAt)} → ${r.signOutAt ? fmtTime(r.signOutAt) : "live"}`
                            : "Not signed in"}
                        </span>
                      </div>
                    );
          ...
          <thead>
            <tr className="text-left text-xs uppercase tracking-wider text-muted-foreground border-b border-border bg-muted/30">
              ...
              <th className="px-4 py-2.5">
                <LogIn className="inline h-3.5 w-3.5 mr-1" />
                Signed in
              </th>
              <th className="px-4 py-2.5">
              ...
              return (
                <tr key={`${r.userId}-${r.shift}`}>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center gap-1.5 rounded-md border px-2 py-0.5 text-xs font-medium ${live ? "bg-success/15 text-success border-success/30" : "bg-info/15 text-info border-info/30"}`}
                    >
                      <span
                        className={`h-1.5 w-1.5 rounded-full ${live ? "bg-success animate-pulse" : "bg-info"}`}
                      />
              ...
              return (
                <tr key={d} className={d === today ? "bg-primary/5" : ""}>
                  <td className="px-4 py-3 text-sm font-medium">
                    {d}
                    {d === today && <span className="ml-2 text-xs text-primary">Today</span>}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1.5">
                      {m?.engineers.map((id) => (
                        <span key={id} className="text-xs rounded-full bg-muted px-2 py-0.5">
                          {userById(id).split(" ")[0]}
                        </span>
                      ))}
                    </div>
