...
interface ReportItem {
  ...
  by: string;
  at: string;
  type: "Incident Report" | "Activity Report" | "RCA";
}
...
export function DetailDrawer({
  ...
  const reports = ensure(seedReports, item.id, () =>
    kind === "incident"
      ? [
          {
            id: "r1",
  ...
  const activity = ensure(seedActivity, item.id, () => [
    {
      ...
      at: "Today 06:12",
      by: "System",
      text: `${kind === "incident" ? "Incident" : "Task"} created from ${item.source ?? "manual entry"}.`,
    },
    {
      id: "a2",
      at: "Today 06:30",
      by: userById(item.assignee ?? null) || "Unassigned",
      text: "Assignment updated.",
    },
  ]);

  const me = user?.name ?? "Engineer";
  const canEdit = kind === "task" ? canEditTask(user, item) : canWorkIncidents(user);
  ...
  const onUpload = (target: "evidence" | "report") => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!user || !canEdit || !files?.length) return;
    Array.from(files).forEach((f) => {
      if (target === "evidence") {
        const kind: EvidenceItem["kind"] = f.type.startsWith("image")
          ? "image"
          : f.name.endsWith(".log")
            ? "log"
            : "doc";
        evidence.unshift({
          id: `e${Date.now()}-${f.name}`,
          name: f.name,
      ...
      activity.unshift({
        ...
        at: "just now",
        by: me,
        text: `Attached ${target === "report" ? reportType : "evidence"}: ${f.name}`,
      });
      recordAuditLog({
        actorId: user.id,
        action: `${kind}.${target}.attach`,
        entityType: kind,
        entityId: item.id,
        after: { name: f.name },
      });
      ...
      <SheetContent className="w-full sm:max-w-2xl overflow-y-auto p-0">
        <div className="border-b border-border px-6 py-5 bg-muted/30">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-2 mt-4 text-xs">
            ...
            <Field
              label={kind === "incident" ? "Created" : "Due"}
              value={(kind === "incident" ? item.createdAt : item.dueDate) ?? "—"}
            />
            <Field label="Category" value={item.category ?? "—"} />
            <Field
              label={kind === "incident" ? "Subcategory" : "Type"}
              value={(kind === "incident" ? item.subcategory : item.type) ?? "—"}
            />
          </div>
          {canEdit && (
            <div className="flex items-center gap-2 mt-4">
              ...
              <QuickBtn
                ...
                label="Accept"
                onClick={() => {
                  setStatusDraft("In Progress");
                  toast("Marked In Progress (save to apply)");
                }}
              />
              <QuickBtn
                icon={ArrowUpRight}
                ...
                onClick={() => {
                  ...
                  toast("Marked Escalated (save to apply)");
                }}
              />
            </div>
          )}
        ...
        <Tabs defaultValue="update" className="px-6 pb-6 pt-4">
          <TabsList className="w-full justify-start">
            ...
            <TabsTrigger value="comments">
              Comments{" "}
              <span className="ml-1.5 text-[10px] text-muted-foreground">{comments.length}</span>
            </TabsTrigger>
            <TabsTrigger value="evidence">
              Evidence{" "}
              <span className="ml-1.5 text-[10px] text-muted-foreground">{evidence.length}</span>
            </TabsTrigger>
            {kind === "incident" && (
              <TabsTrigger value="reports">
                Reports{" "}
                <span className="ml-1.5 text-[10px] text-muted-foreground">{reports.length}</span>
              </TabsTrigger>
            )}
          ...
          <TabsContent value="update" className="space-y-4 mt-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  disabled={!canEdit}
                  className="mt-1.5 w-full rounded-md border border-input bg-card px-2 py-2 text-sm disabled:opacity-60"
                >
                  {statusOptions.map((s) => (
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={
                  kind === "incident"
                    ? "Describe current state, mitigations, ETA…"
                    : "Progress notes, blockers, next steps…"
                }
          ...
          <TabsContent value="comments" className="space-y-3 mt-4">
            <div className="flex items-start gap-2">
              <textarea
                ...
                disabled={!canEdit}
                rows={2}
                placeholder={canEdit ? "Add a comment…" : "Read-only for your role"}
                className="flex-1 rounded-md border border-input bg-card px-3 py-2 text-sm resize-none disabled:opacity-60"
              />
              {canEdit && (
                <button
                  onClick={addComment}
                  className="rounded-md bg-primary text-primary-foreground px-3 py-2 text-sm hover:bg-primary/90 inline-flex items-center gap-1.5"
                >
          ...
          <TabsContent value="evidence" className="space-y-3 mt-4">
            <input
              ...
              multiple
              className="hidden"
              onChange={onUpload("evidence")}
            />
            <button
              disabled={!canEdit}
              onClick={() => fileInput.current?.click()}
              className="w-full rounded-md border-2 border-dashed border-border hover:border-primary/50 hover:bg-muted/40 transition-colors p-6 text-center disabled:opacity-60"
            >
              ...
              <div className="text-xs text-muted-foreground mt-0.5">
                Photos, logs, screenshots, docs · max 25 MB each
              </div>
            </button>
            ...
            <TabsContent value="reports" className="space-y-3 mt-4">
              <div className="rounded-md border border-border bg-card p-4">
                <div className="flex items-start gap-2 mb-3">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Attach an <span className="font-medium text-foreground">Incident Report</span>,{" "}
                    <span className="font-medium text-foreground">Activity Report</span>, or{" "}
                    <span className="font-medium text-foreground">RCA</span> document. Reports are
                    tracked separately from raw evidence and visible in the incident audit trail.
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                  ...
                  <div className="flex items-end">
                    <input
                      ...
                      type="file"
                      className="hidden"
                      onChange={onUpload("report")}
                    />
                    <button
                      disabled={!canEdit}
                      onClick={() => reportInput.current?.click()}
                      className="w-full rounded-md bg-primary text-primary-foreground px-3 py-2 text-sm hover:bg-primary/90 inline-flex items-center justify-center gap-1.5 disabled:opacity-60"
                    >
